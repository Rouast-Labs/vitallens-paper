...
The demographics of the participants in PROSIT are given in Figure \ref{fig:prosit-demographics}.
Most participants in PROSIT are under 40 years old, and there are more male than female participants.
As we show in Section \ref{results}, this is not an issue in practice.
However, the skin type diversity of PROSIT is also lacking, with too many type 2 and too few type 5 and 6 participants.
This is in fact an issue.
To avoid a skin type bias in our model, we supplement our training data with the \textit{Vital Videos Ghana} training set which has mostly type 5 and 6 participants.
...
\begin{figure}[h!]
    ...
    \end{subfigure}
    \hfill
    \caption{Participant demographics in PROSIT}
    \label{fig:prosit-demographics}
\end{figure}

\paragraph{Vitals diversity.}
Distributions of the vitals in PROSIT are given in Figure \ref{fig:prosit-vitals-histogram}.
	...
	\begin{subfigure}{0.48\textwidth}
    	...
    	\begin{tikzpicture}
        	\begin{axis}[
            	...
            	\addplot[ybar,bar width=10pt,fill=blue!60,opacity=0.8] table[x index=0, y index=1] {data/bp_sys_histogram.csv};
            	\addlegendentry{SBP}
            	\addplot[ybar,bar width=7pt,fill=red!60,opacity=0.7] table[x index=0, y index=1] {data/bp_dia_histogram.csv};
            	\addlegendentry{DBP}
        	\end{axis}
    	\end{tikzpicture}
    \end{subfigure}
    ...
    \begin{subfigure}{0.48\textwidth}
    	...
    	\begin{tikzpicture}
        	\begin{axis}[
            	...
            	\addplot[fill=blue!60,opacity=0.8] table[x index=0, y index=1] {data/spo2_histogram.csv};
        	\end{axis}
    	\end{tikzpicture}
    \end{subfigure}
    \caption{Distributions of chunk summary vitals in PROSIT}
    \label{fig:prosit-vitals-histogram}
\end{figure}
